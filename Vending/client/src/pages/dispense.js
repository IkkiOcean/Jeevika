...
useEffect(()=>{
    let medicines
    axios.get(`http://127.0.0.1:5000/fetch-order?id=${id}`).then((res) => {
        console.log(res);
        ...
        medicines = res.data.medicine_id.map((medicine_id, i) => {
          ...
          axios.post(`http://127.0.0.1:5000/dispense`, medicines).then((res) => {
                console.log(res);
                console.log(res.data);
                navigator("/thank");
    });
...
// await axios.get(`http://127.0.0.1:5000/fetch-order?id=${id}`).then((res) => {
//       console.log(res);
//       console.log(res.data);
...
//       });
  return (
    ...
    <h3 className="text-3xl">Dispensing Medicines</h3>
  </div>
  )
}
